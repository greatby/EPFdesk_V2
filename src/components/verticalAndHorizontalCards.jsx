  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto overflow-hidden">
        ...
        >
          {verticalCards.map(({ title, subtitle, icon, bubble, bg }, index) => (
            <SwiperSlide key={index}>
              <div
                className={`relative ${bg} min-h-[380px] h-full p-10 rounded-3xl shadow-xl transition hover:-translate-y-2 hover:shadow-2xl flex flex-col items-center justify-between`}
              >
...
const HorizontalCard = ({ title, subtitle, icon, bubble, bg }) => (
  <div
    className={`relative ${bg} p-8 rounded-3xl shadow-xl transition hover:-translate-y-2 hover:shadow-2xl flex flex-col md:flex-row items-center md:items-start`}
  >
    <div className="absolute inset-0 pointer-events-none">
      <FloatingCircles />
    </div>
